module.exports = function (app) {
    ...
    Query.csv = function (params) {
        ...
        let mapper = {
            ...
            "bi_ce_prodes_desmatamento_pontos_campo_fip": "select p.gid as progid, p.geom as progeom, p.county as procounty, p.uf, p.cd_geocmu, p.view_date as pview_date, p.areamunkm as pareamunkm, p.sucept_desmat as psucept_desmat, pc.gid pontogid, pc.geom as pontogeom, pc.campo as pontocampo from prodes_cerrado p inner join pontos_campo pc on pc.prodes_id = p.gid where 1=1",
            "bi_ce_prodes_desmatamento_abc_fip": "select abc.*, p.gid, p.county, p.uf, p.view_date, p.sucept_desmat, p.areamunkm from prodes_fip_abc abc inner join prodes_cerrado p on abc.prodes_id = p.gid where 1=1"
        }
