module.exports = function (app) {
	...
	Query.field = function (params) {
		...
		return [
			...
			{
				id: 'areas_especiais',
				sql: "SELECT pd.ti_dist, ST_AREA(ST_Intersection(ti.geom,desmatamento.geom)::GEOGRAPHY) / 1000000.0 as area_desmat_ti, pd.ti_nom, pd.ti_gid,"
					+ " rect_bbox(ST_Envelope(ST_Collect(ti.geom,desmatamento.geom))) as bbox_ti,"
					+ " pd.q_dist, ST_AREA(ST_Intersection(q.geom, desmatamento.geom)::GEOGRAPHY) / 1000000.0 as area_desmat_q, pd.q_nom, pd.q_gid,"
					+ " rect_bbox(ST_Envelope(ST_Collect(desmatamento.geom,q.geom))) As bbox_q,"
					+ " pd.ucpi_dist, ST_AREA(ST_Intersection(ucpi.geom, desmatamento.geom)::GEOGRAPHY) / 1000000.0 as area_desmat_ucpi, pd.ucpi_nom, pd.ucpi_gid,"
					+ " rect_bbox(ST_Envelope(ST_Collect(desmatamento.geom,ucpi.geom))) As bbox_ucpi,"
					+ " pd.ucus_dist, ST_AREA(ST_Intersection(ucus.geom, desmatamento.geom)::GEOGRAPHY) / 1000000.0 as area_desmat_ucus, pd.ucus_nom, pd.ucus_gid,"
					+ " rect_bbox(ST_Envelope(ST_Collect(desmatamento.geom,ucus.geom))) As bbox_ucus"
					// + " pd.ap_dist, ST_AREA(ST_Intersection(ap.geom, desmatamento.geom)::GEOGRAPHY) / 1000000.0 as area_desmat_ap, pd.ap_nom, pd.ap_gid,"
					// + " rect_bbox(ST_Envelope(ST_Collect(desmatamento.geom,ap.geom))) As bbox_ap"
