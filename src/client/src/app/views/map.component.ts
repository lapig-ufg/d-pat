...
export class MapComponent implements OnInit {
	...
	onCityRowSelect(event) {
		...
		this.http
			.get(SEARCH_URL, { params: PARAMS.set("key", name) })
			.subscribe(result => {
				let ob = result[0];
	...
	private updateTexts() {
		...
		console.log("DESCRIPTOR: ", this.descriptorText);

		var textlangurl = "/service/map/textreport?lang=" + this.language;

		this.http.get(textlangurl).subscribe(result => {
	...
	private updateCharts() {
		...
		var statesUrl =
			"/service/deforestation/states" + this.getServiceParams();
		var citiesUrl =
			"/service/deforestation/cities" + this.getServiceParams();
		var citiesIllegal =
			"/service/deforestation/illegal" + this.getServiceParams();
				...
				datasets: [
					{
						...
						data: timeseriesResult["series"].map(
		...
		if (!this.isFilteredByCity) {
			...
			if (this.desmatInfo.year >= 2013) {
				this.http.get(citiesIllegal).subscribe(citiesIllegalResult => {
					this.chartResultCitiesIllegalAPP =
						citiesIllegalResult["resultAPP"];
					this.chartResultCitiesIllegalRL =
						citiesIllegalResult["resultRL"];
				});
			}
		}
	}
	...
	updateRegion(region) {
		let prodes = this.layersNames.find(
			element => element.id === "desmatamento_prodes"
		);
	...
	private createMap() {
		...
		let deter = this.layersNames.find(
			element => element.id === "desmatamento_deter"
		);
	...
	private callbackPointerMoveMap(evt) {
		...
		if (prodes.visible || deter.visible) {
			...
			if (
				prodes.selectedType == "bi_ce_prodes_desmatamento_100_fip" ||
				deter.selectedType == "bi_ce_deter_desmatamento_100_fip"
			) {
				isOficial = true;
			}
			...
			if (
				prodes.selectedType ==
					"bi_ce_prodes_desmatamento_pontos_campo_fip" ||
				deter.selectedType ==
					"bi_ce_deter_desmatamento_pontos_campo_fip"
			) {
				isCampo = true;
			}
			...
			if (isMunicipio) {
				if (this.utfgridmunicipio) {
					this.utfgridmunicipio.forDataAtCoordinateAndResolution(
						coordinate,
						viewResolution,
						function(data) {
							if (data) {
								...
								if (
									...
									this.infodataMunicipio.area_app_show =
										this.infodataMunicipio.area_app == ""
											? this.minireportText
													.undisclosed_message
											: (
													"" +
													Math.round(
														this.infodataMunicipio
															.area_app * 100
													) /
														100 +
													" km²"
											  ).replace(".", ",");
									this.infodataMunicipio.area_rl_show =
										this.infodataMunicipio.area_rl == ""
											? this.minireportText
													.undisclosed_message
											: (
													"" +
							...
							function(data) {
								if (data) {
									...
									if (
										...
										if (data.origin_table == "PRODES") {
											window.document.body.style.cursor =
												"pointer";
											this.infodata = data;
											this.infodata.dataFormatada =
												this.infodata.data_detec == ""
													? this.minireportText
															.undisclosed_message
											...
											this.infodata.sucept_desmatFormatada =
												this.infodata.sucept_desmat ==
												null
													? this.minireportText
									...
									if (
										deter.visible &&
										deter.selectedType ==
											"bi_ce_deter_desmatamento_100_fip"
									) {
			...
			if (isCampo) {
				if (
					...
					if (this.utfgridCampo) {
						coordinate = this.map.getEventCoordinate(
							evt.originalEvent
						);
						var viewResolution = this.map.getView().getResolution();

						this.utfgridCampo.forDataAtCoordinateAndResolution(
							coordinate,
							viewResolution,
							function(data) {
								if (data) {
									...
									if (
										...
										if (data.origin_table == "PRODES") {
											...
											this.infodataCampo.dataFormatada =
												this.infodataCampo.data_detec ==
												""
													? this.minireportText
															.undisclosed_message
													: this.datePipe.transform(
											...
											this.infodataCampo.sucept_desmatFormatada =
												this.infodataCampo
													.sucept_desmat == ""
													? this.minireportText
															.not_computed_message
						...
						function(data) {
							if (data) {
								...
								if (
									...
									this.dataForDialog.year = new Date(
										this.dataForDialog.data_detec
									).getFullYear();
						...
						function(data) {
							if (data) {
								...
								if (
									...
									if (
										this.dataForDialog.origin_table
											.toUpperCase === "PRODES"
									) {
										this.dataForDialog.year = this.selectedTimeFromLayerType(
											"bi_ce_prodes_desmatamento_100_fip"
										).year;
									} else {
										this.dataForDialog.year = new Date(
	...
	private getTileJSON() {
		let text =
			"((origin_table = 'prodes' AND " +
			this.selectedTimeFromLayerType("bi_ce_prodes_desmatamento_100_fip")
				.value +
			")" +
			" OR " +
			"(origin_table = 'deter' AND " +
			this.selectedTimeFromLayerType("bi_ce_deter_desmatamento_100_fip")
				.value +
			"))";
	...
	private getTileJSONCampo() {
		...
		return {
			...
			grids: [
				this.returnUTFGRID(
					"bi_ce_info_utfgrid_pontos_campo_fip",
					text,
					"{x}+{y}+{z}"
				),
			],
	...
	private returnUTFGRID(layername, filter, tile) {
		return (
			...
			"&mode=tile&tile=" +
			tile +
			"&tilemode=gmap&map.imagetype=utfgrid"
		);
	}
	...
	private updateSourceLayer(layer) {
		...
		if (
			layer["value"] === "bi_ce_prodes_desmatamento_100_fip" ||
			layer["value"] === "prodes_por_region_fip"
		) {
	...
	baseLayerChecked(base, e) {
		for (let basemap of this.basemapsNames) {
			if (base.value == basemap.value && e.checked) {
				...
				this[
					this.descriptor.basemaps[0].defaultBaseMap
				].layer.setVisible(true);
				if (
					basemap.value != this.descriptor.basemaps[0].defaultBaseMap
				) {
					this[basemap.value].layer.setVisible(false);
		...
		if (prodes.visible || deter.visible) {
			if (
				...
				if (this.utfgridsource) {
					...
					this.utfgridsource.tileUrlFunction_ = _ol_TileUrlFunction_.createFromTemplates(
						tileJSON.grids,
						this.utfgridsource.tileGrid
					);
			...
			if (
				...
				if (this.utfgridCampo) {
					...
					this.utfgridCampo.tileUrlFunction_ = _ol_TileUrlFunction_.createFromTemplates(
						tileJSONCampo.grids,
						this.utfgridCampo.tileGrid
					);
			...
			if (prodes.selectedType == "prodes_por_region_fip") {
				if (this.utfgridmunicipio) {
					...
					this.utfgridmunicipio.tileUrlFunction_ = _ol_TileUrlFunction_.createFromTemplates(
						tileJSONMunicipio.grids,
						this.utfgridmunicipio.tileGrid
					);
	...
	private updateDescriptor() {
		this.descriptor.type = this.descriptorText.type_of_information_label[
			this.language
		];
		...
		for (let group of this.descriptor.groups) {
			...
			for (let layer of group.layers) {
				...
				layer.label = this.descriptorText[group.id].layers[
					layer.id
				].label[this.language];

				for (let layerType of layer.types) {
					if (
						this.descriptorText[group.id].layers[
							layer.id
						].hasOwnProperty("types")
					) {
						if (
							this.descriptorText[group.id].layers[
								layer.id
							].types[layerType.value].hasOwnProperty(
								"view_value"
							)
						) {
							layerType.Viewvalue = this.descriptorText[
								group.id
							].layers[layer.id].types[
								layerType.value
							].view_value[this.language];
						}
						if (
							this.descriptorText[group.id].layers[
								layer.id
							].types[layerType.value].hasOwnProperty("timelabel")
						) {
							layerType.timeLabel = this.descriptorText[
								group.id
							].layers[layer.id].types[layerType.value].timelabel[
								this.language
							];
						}
						...
						if (layerType.times) {
							for (let time of layerType.times) {
								if (
									...
									time.Viewvalue = this.descriptorText[
										group.id
									].layers[layer.id].types[
										layerType.value
									].times[time.value][this.language];
							}
						}
					}
				}
		...
		for (let basemap of this.descriptor.basemaps) {
			for (let types of basemap.types) {
				types.viewValue = this.descriptorText.basemaps.types[
					types.value
				][this.language];
			}
		}
		...
		for (let limits of this.descriptor.limits) {
			for (let types of limits.types) {
				types.Viewvalue = this.descriptorText.limits.types[types.value][
					this.language
				];
			}
		}
	...
	public onFileComplete(data: any) {
		...
		if (data.features.length > 1) {
			...
			if (data.features[0].hasOwnProperty("properties")) {
				...
				this.layerFromUpload.visible = true;
				this.layerFromUpload.label =
					data.features[0].properties[auxlabel];
				this.layerFromUpload.layer = data;
			} else {
				this.layerFromUpload.visible = true;
				this.layerFromUpload.label = data.name;
		...
		this.http.get(fieldPhotosUrl).subscribe(
			result => {
				...
				if (this.infoDesmat.classefip == null) {
					...
					this.infoDesmat.pathclassefip =
						"/assets/metric/classe" +
						this.infoDesmat.classefip +
						".png";
				}
				...
				this.carData.forEach(element => {
					element.metaData.dataRefFormatada =
						element.metaData.dataRef == ""
							? this.textOnDialog.car_tab.undisclosed_message_car
							: this.data.datePipe.transform(
									new Date(element.metaData.dataRef),
									"dd/MM/yyyy"
							  );
					//element.metaData.area_car = element.metaData.area_car / 100.0  //converte HA to Km2
					element.metaData.percentDesmat =
						"" +
						(
							(
								(element.metaData.area_desmatada /
									element.metaData.area_car) *
								100
							).toFixed(2) + "%"
						).replace(".", ",");
					element.metaData.percentRL =
						"" +
						(
							(
								(element.metaData.area_desmat_rl /
									element.metaData.area_reserva_legal_total) *
								100
							).toFixed(2) + "%"
						).replace(".", ",");
					element.metaData.percentAPP =
						"" +
						(
							(
								(element.metaData.area_desmat_app /
									element.metaData.area_app_total) *
								100
							).toFixed(2) + "%"
						).replace(".", ",");
				...
				for (let i = 0; i < this.urlsLandSat.length; i++) {
					const album = {
						...
						caption:
							this.textOnDialog.historico_amostral_landsat
								.caption + this.urlsLandSat[i].year,
				...
				this.dataBfast.prob_Formatada =
					this.dataBfast.pct_bfast == null
						? this.textOnDialog.analise_automatica.not_computed
						: (
								"" +
								this.dataBfast.pct_bfast.toFixed(2) +
								"%"
						  ).replace(".", ",");
				...
				const dfast = {
					...
					caption:
						this.dataBfast.prob_Formatada +
						this.textOnDialog.analise_automatica.caption_bfast,
				...
				this.dataSuscept.sucept_desmatFormatada =
					this.dataSuscept.prob_suscept == null
						? this.textOnDialog.analise_automatica.not_computed
						: (
								"" +
								(this.dataSuscept.prob_suscept * 100).toFixed(
									2
								) +
								"%"
						  ).replace(".", ",");
				const dsuscept = {
					...
					caption:
						this.textOnDialog.analise_automatica.caption_suscept +
						this.dataSuscept.sucept_desmatFormatada,
			...
			() => {
				...
				for (let index = 0; index < this.dataCampo.length; index++) {
					...
					this.infoVisita.dataFormatada =
						this.infoVisita.data_visita == ""
							? this.textOnDialog.campo_tab.caption_undisclosed
							: this.infoVisita.data_visita;

					for (
						let foto = 0;
						foto < element.fotos_camera.length;
						foto++
					) {
						this.galleryCamera.push({
							small:
								"/service/map/field/fotos_camera/" +
								element.campo_id +
								"/" +
								element.fotos_camera[foto],
							medium:
								"/service/map/field/fotos_camera/" +
								element.campo_id +
								"/" +
					...
					for (
						let foto = 0;
						foto < element.fotos_drone.length;
						foto++
					) {
						this.galleryDrones.push({
							small:
								"/service/map/field/fotos_drone/" +
								element.campo_id +
								"/" +
								element.fotos_drone[foto],
							medium:
								"/service/map/field/fotos_drone/" +
								element.campo_id +
								"/" +
					...
					for (
						let video = 0;
						video < element.videos_drone.length;
						video++
					) {
						this.galleryVideos.push({
							small:
								"/service/map/field/videos_drone/" +
								element.campo_id +
								"/" +
								element.videos_drone[video],
							medium:
								"/service/map/field/videos_drone/" +
								element.campo_id +
								"/" +
